...
from layout import genetic, alphabetize, layout_score, draw_image, to_pairs
import os, time
app = Flask(__name__)
...
def layout():
    if request.method == 'POST':
        ...
        for i in range(int(max_gen)):
            vals = genetic(string, sums, same_max_count, curr_max, gen, final_layouts, removed_chars, pairs)
            sums = vals[0]
        ...
        for layout in final_layouts:
            if layout_score(layout, string, pairs) == maxSum:
                max_layout = layout
                break
        print(max_layout)
